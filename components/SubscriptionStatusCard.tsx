...
const SubscriptionStatusCard: React.FC<SubscriptionStatusCardProps> = ({ renewalDate, compact, collapsed }) => {
    ...
    if (collapsed) {
        const label = daysLeft > 0 ? `${daysLeft} jours restants` : daysLeft === 1 ? `${daysLeft} jour restant` : "Expiré";
        return (
            <button title={label} aria-label={label} className={`flex items-center justify-center px-2 py-2 rounded-lg mx-0 cursor-pointer w-full ${statusClass}`}>
